...
{	
	...
	if (!target_timestamp.isValid() || target_timestamp.isNever()) {
		mprintf(("Nonsense timestamp in multi_ship_record_find_frame_and_time of %s. Get ~~Allender~~ Cyborg!\n", (target_timestamp.isValid()) ? "isNever" : "NOT isValid"));
		return frame;
	};
...
vec3d multi_ship_record_lookup_position(object* objp, int frame, float percent) 
{
	...
	if (next_frame >= MAX_FRAMES_RECORDED){
		next_frame = 0;
	}
	...
	vm_vec_sub(&temp_vector, &Oo_info.frame_info[objp->net_signature].positions[next_frame], &Oo_info.frame_info[objp->net_signature].positions[frame]);
	vm_vec_scale_add(pos, &Oo_info.frame_info[objp->net_signature].positions[frame], &temp_vector, percent);

	return Oo_info.frame_info[objp->net_signature].positions[frame];
}

// Quick lookup for the record of orientation.
matrix* multi_ship_record_lookup_orientation(object* objp, int frame, float percent) 
{
	Assertion(objp != nullptr, "nullptr given to multi_ship_record_lookup_position. \nThis should be handled earlier in the code, please report!");
	if (objp == nullptr) {
	...
	return vm_interpolate_matrices(&Oo_info.frame_info[objp->net_signature].orientations[frame], Oo_info.frame_info[objp->net_signature].orientations[next_frame], percent);
}

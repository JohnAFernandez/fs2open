...
void LoadoutDialogModel::setShipEnablerVariables(SCP_vector<SCP_string> variablesIn, bool enabled)
{
		...
		for (auto& nameIn : variablesIn) {
			...
			if (!found) {
				_teams[_currentTeam].varShips.emplace_back(
					get_index_sexp_variable_name(nameIn.c_str()),
					true,
...
void LoadoutDialogModel::setWeaponEnablerVariables(SCP_vector<SCP_string> variablesIn, bool enabled)
{
	for (auto& nameIn : variablesIn) {
